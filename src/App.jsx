...
const REGIONS = {
  "East Asia":{
    ...
    routingHubs:[
      ...
      {code:"TPE",name:"Taipei Taoyuan",uaDirect:true,note:"UA direct from SFO/LAX. Connect to BR/CI/JX/IT to Japan.",connPartners:["BR","CI","JX","IT","JL"]},
  ...
  "Southeast Asia":{
    ...
    routingHubs:[
      ...
      {code:"MNL",name:"Manila",uaDirect:true,note:"UA direct from SFO/LAX. PR (J class!) onward to SE Asia cities.",connPartners:["PR","CX"]},
  ...
  "Europe":{
    ...
    routingHubs:[
      {code:"LHR",name:"London Heathrow",uaDirect:true,note:"UA from SFO/LAX/EWR/IAD/ORD/IAH/DEN/BOS. Also BA (J!) and AA (J!) direct.",connPartners:["BA","AA","IB","EI","VS","AF","KL"]},
      {code:"FRA",name:"Frankfurt",uaDirect:true,note:"UA from SFO/LAX/EWR/IAD/ORD/IAH/DEN. Also LH (J!) direct.",connPartners:["LH","LX","OS","SK","SN","TP","AZ"]},
      {code:"MUC",name:"Munich",uaDirect:true,note:"UA from SFO/EWR/IAD/ORD. Also LH (J!) direct.",connPartners:["LH","OS","SK"]},
      {code:"CDG",name:"Paris CDG",uaDirect:true,note:"UA from SFO/EWR/IAD. Also AA (J!) direct. AF/KL economy hub.",connPartners:["AF","KL","DL"]},
      {code:"AMS",name:"Amsterdam Schiphol",uaDirect:true,note:"UA from SFO/IAD/IAH. KL economy mega-hub.",connPartners:["KL","DL"]},
      {code:"ZRH",name:"Zurich",uaDirect:true,note:"UA from SFO/EWR. LX (J!) direct.",connPartners:["LX","EW"]},
      ...
      {code:"LIS",name:"Lisbon",uaDirect:true,note:"UA from EWR. TP economy hub.",connPartners:["TP"]},
      {code:"ATH",name:"Athens",uaDirect:true,note:"UA from EWR (seasonal).",connPartners:["A3"]},
      {code:"EDI",name:"Edinburgh",uaDirect:true,note:"UA from EWR (seasonal).",connPartners:[]},
      {code:"IST",name:"Istanbul",uaDirect:true,note:"UA from IAD/EWR. TK economy mega-hub.",connPartners:["TK"]},
      {code:"VIE",name:"Vienna",uaDirect:false,note:"OS (J class) from US gateways.",connPartners:["OS","LO"]},
      {code:"HEL",name:"Helsinki",uaDirect:false,note:"AY (J class) from US gateways.",connPartners:["AY","SK","FI"]},
      {code:"WAW",name:"Warsaw",uaDirect:false,note:"LO (J class) from ORD/EWR/JFK.",connPartners:["LO"]},
      {code:"DOH",name:"Doha",uaDirect:false,note:"QR (J class) from US.",connPartners:["QR"]},
    ],
    hubStrategy:[
  ...
  "Middle East":{
    ...
    routingHubs:[
      ...
      {code:"AUH",name:"Abu Dhabi",uaDirect:false,note:"EY (J class) from JFK/IAD/ORD/LAX.",connPartners:["EY"]},
      {code:"DXB",name:"Dubai",uaDirect:false,note:"EK (economy) from JFK/IAD/LAX/SFO/IAH/DFW/BOS/ORD/SEA.",connPartners:["EK"]},
      {code:"IST",name:"Istanbul",uaDirect:true,note:"UA from IAD/EWR. TK economy mega-hub.",connPartners:["TK"]},
      ...
      {code:"AMM",name:"Amman",uaDirect:false,note:"RJ (J class) from ORD/JFK/DFW.",connPartners:["RJ"]},
  ...
  "South Asia":{
    ...
    routingHubs:[
      ...
      {code:"DOH",name:"Doha Hamad",uaDirect:false,note:"QR (J class) from many US cities.",connPartners:["QR"]},
      {code:"AUH",name:"Abu Dhabi",uaDirect:false,note:"EY (J class) from JFK/IAD/ORD/LAX.",connPartners:["EY"]},
      {code:"DXB",name:"Dubai",uaDirect:false,note:"EK (economy) from many US cities.",connPartners:["EK"]},
  ...
  "Oceania":{
    ...
    routingHubs:[
      ...
      {code:"MEL",name:"Melbourne",uaDirect:true,note:"UA from SFO/LAX (seasonal). QF (J class) from LAX.",connPartners:["QF","VA"]},
  ...
  "South America":{
    ...
    routingHubs:[
      ...
      {code:"EZE",name:"Buenos Aires Ezeiza",uaDirect:true,note:"UA from IAH/EWR. AA from MIA/DFW.",connPartners:["LA","AA"]},
      {code:"SCL",name:"Santiago",uaDirect:true,note:"UA from IAH. AA from MIA/DFW. LA economy from US.",connPartners:["LA"]},
      ...
      {code:"PTY",name:"Panama City Tocumen",uaDirect:true,note:"UA from IAH/EWR. CM economy mega-hub.",connPartners:["CM"]},
  ...
  "Africa":{
    ...
    routingHubs:[
      {code:"ADD",name:"Addis Ababa Bole",uaDirect:false,note:"ET (economy) from IAD direct. Ethiopian mega-hub.",connPartners:["ET"]},
      {code:"JNB",name:"Johannesburg OR Tambo",uaDirect:true,note:"UA from EWR (direct, new route). SA economy connections.",connPartners:["SA"]},
      {code:"ACC",name:"Accra Kotoka",uaDirect:true,note:"UA from IAD/EWR (direct). West Africa gateway.",connPartners:["ET"]},
      {code:"LOS",name:"Lagos Murtala Muhammed",uaDirect:false,note:"Connect via ADD (ET), IST (TK), LHR (BA), or ACC (UA).",connPartners:["ET","TK","BA"]},
      {code:"IST",name:"Istanbul",uaDirect:true,note:"UA from IAD/EWR. TK economy mega-hub — flies to 30+ African cities.",connPartners:["TK"]},
...
function calcTier(isDirect, isLate, remaining, open) {
  if (open === null || open === undefined) return 3; // no data yet
  if (open === 0) return 4;
...
function sortConnections(conns){
  return [...conns].sort((a,b)=>{
    ...
    const aLayScore=aLay<2?aLay+100:aLay;
    ...
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",background:"#f5f4f0",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#fff",border:"1px solid #e5e7eb",borderRadius:16,padding:"40px 36px",maxWidth:380,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <p style={{fontSize:12,color:"#64748b"}}>Sign in to sync trips across devices</p>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <input value={email} onChange={e=>setEmail(e.target.value)} placeholder="Email" type="email"
            style={{fontSize:13,padding:"10px 12px",borderRadius:8,border:"1.5px solid #e5e7eb",outline:"none",width:"100%",boxSizing:"border-box",color:"#1e293b"}}/>
...
function Loading(){return <div style={{padding:60,textAlign:"center",color:"#64748b",fontFamily:"monospace",background:"#f5f4f0",minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center"}}>Loading…</div>;}
  ...
  <div style={{background:"#1e293b",borderBottom:"1px solid #334155",padding:"12px 20px",position:"sticky",top:0,zIndex:100,backdropFilter:"blur(12px)"}}>
    <div style={{maxWidth:1140,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,cursor:"pointer"}} onClick={goHome}>
        ...
        <div style={{fontSize:8,color:"#94a3b8",fontFamily:"monospace",letterSpacing:".1em"}}>ALASKA ZED/MIBA · {J_PARTNERS.length} J PARTNERS · {Y_PARTNERS.length} Y PARTNERS</div></div>
      </div>
...
function Dashboard({trips,open,del,onNew}){
  ...
  return (<div>
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
      <div><h2 style={{fontSize:22,fontWeight:800,margin:"0 0 4px",color:"#111827"}}>Your Trips</h2><p style={{fontSize:12,color:"#64748b",margin:0}}>{list.length} trip{list.length!==1?"s":""} · J class on {J_PARTNERS.length} airlines · Y class on {Y_PARTNERS.length} airlines</p></div>
    ...
    {list.length===0 ? (
      <div style={{textAlign:"center",padding:"60px 20px",background:"#ffffff",borderRadius:14,border:"1px solid #e5e7eb"}}>
        ...
        <div style={{fontSize:13,color:"#64748b",marginBottom:20}}>Plan a non-rev standby trip with your full ZED/MIBA agreement network.</div>
            ...
            <div key={t.id} style={{background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:12,cursor:"pointer",transition:"border-color .15s"}}
              ...
              <div onClick={()=>open(t.id)} style={{padding:"16px 18px"}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:8}}>
                  <span style={{fontSize:10,padding:"3px 10px",borderRadius:99,background:"#f3f4f6",color:"#475569",fontWeight:600,fontFamily:"monospace"}}>{t.cabin==="J"?"Business":"Economy"}</span>
                  <span style={{fontSize:10,color:"#64748b"}}>{t.travelDate||"Flexible"}</span>
                </div>
                <div style={{fontSize:20,fontWeight:900,color:"#111827",marginBottom:3,letterSpacing:"-.02em"}}>{t.origin}  {t.destination}</div>
                <div style={{fontSize:12,color:"#64748b",marginBottom:8}}>{t.name}</div>
              ...
              <div style={{borderTop:"1px solid #e5e7eb",padding:"6px 18px",display:"flex",justifyContent:"flex-end",gap:6}}>
                {confirmId===t.id ? (<>
                  <button onClick={e=>{e.stopPropagation();setConfirmId(null);}} style={{fontSize:10,color:"#64748b",background:"none",border:"none",cursor:"pointer",padding:"4px 8px"}}>Cancel</button>
...
function NewTrip({onCancel,onCreate}){
  ...
  const [travelers,setTrav]=useState("1");
  ...
  return(
    <div style={{maxWidth:560,margin:"0 auto"}}>
      ...
      <div style={{background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:14,padding:"24px 28px",display:"flex",flexDirection:"column",gap:20}}>
          ...
          <FG label="Cabin Priority (trunk leg)">
            <div style={{display:"flex",gap:6}}>
              {[{v:"J",l:"Business (J)",sub:`${J_PARTNERS.length} airlines`},{v:"Y",l:"Economy (Y)",sub:`${Y_PARTNERS.length} airlines`}].map(o=>(
                <button key={o.v} onClick={()=>setCabin(o.v)} style={{flex:1,padding:"10px",borderRadius:8,border:"1.5px solid "+(cabin===o.v?"#2563eb":"#d4d4d8"),background:cabin===o.v?"#eff6ff":"transparent",color:cabin===o.v?"#2563eb":"#475569",cursor:"pointer",fontSize:12,fontWeight:cabin===o.v?700:400}}>
        ...
        <FG label="Trunk Airline (optional)">
        ...
        <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:4}}>
          ...
          <Btn onClick={()=>onCreate({
            ...
            travelDate:date, flexDays, cabin, travelers:parseInt(travelers)||1,
          ...
          {matches.map((m,i)=>(
            <div key={i} onMouseDown={()=>handleSelect(m)} style={{padding:"10px 14px",cursor:"pointer",borderBottom:"1px solid #f5f5f5",display:"flex",justifyContent:"space-between",alignItems:"center"}}
              ...
              <div>
                ...
                <span style={{fontSize:11,color:"#64748b",marginLeft:6}}>{m.country}</span>
              </div>
...
function FG({label,children}){return <div><div style={{fontSize:10,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:".06em",marginBottom:6,fontFamily:"monospace"}}>{label}</div>{children}</div>;}
...
function Research({trip,onDone,onSkip}){
  ...
  async function run(){
    ...
    let hubArray = [];
    if (destCoords) {
      const regionalPriorityHubs = matchedData && matchedData.routingHubs ? matchedData.routingHubs.map(h => h.code) : [];
...
function Tracker({trip,onUpdate,onReSearch,goHome}){
  ...
  const remColor=(r)=>r===null?"#374151":r>=5?"#059669":r>=2?"#2563eb":r>=0?"#d97706":"#dc2626";
  const clearMsg=(open,listed)=>{
    const r=getRemaining(open,listed);const tv=trip.travelers||1;
  ...
  return(<div>
    ...
    {addingRoute&&(<div style={{background:"#fff",border:"1.5px solid #2563eb",borderRadius:12,padding:"18px 20px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        ...
        <Btn onClick={()=>setAddingRoute(false)} dim>Close</Btn>
      </div>
      ...
      {!arDirect&&<div style={{borderTop:"1px solid #e5e7eb",paddingTop:10,marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:700,color:"#64748b",marginBottom:8}}>CONNECTION FROM HUB</div>
    ...
    {enriched.filter(r=>r.open===null||r.open>0).length>0&&(
      <div style={{marginBottom:16}}>
        <div style={{fontSize:9,fontWeight:700,color:"#64748b",letterSpacing:".08em",textTransform:"uppercase",fontFamily:"monospace",marginBottom:8}}>Top options</div>
    ...
    {filtered.length === 0 && routes.length > 0 ? (
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {filtered.map((r,ri)=>{
          ...
          const carrierCode = (r.trunkCarrier || r.airline || "").match(/[A-Z0-9]{2}/)?.[0]?.toUpperCase() || "";
            ...
            <div style={{background:tm.bg,padding:"14px 16px",display:"flex",flexDirection:"column",gap:12,borderBottom:`1px solid ${tm.border}`}}>
              <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,minWidth:70}}>
                  <span style={{fontSize:9,color:"#64748b",fontFamily:"monospace"}}>#{ri+1}</span>
                ...
                <div style={{flex:1}}>
                  <div style={{display:"flex",alignItems:"baseline",gap:8,flexWrap:"wrap",marginBottom:4}}>
                    ...
                    <span style={{fontSize:11,color:"#475569",fontFamily:"monospace"}}>{a?.name || r.trunkCarrier || r.airline}</span>
                    {a?.alliance && <span style={{fontSize:9,padding:"1px 6px",borderRadius:99,background:"#f3f4f6",color:"#475569"}}>{a.alliance}</span>}
                    <span style={{fontSize:10,background:"#f3f4f6",color:"#475569",padding:"1px 7px",borderRadius:99,fontFamily:"monospace"}}>{r.aircraft}</span>
                  ...
                  <div style={{marginTop:8, padding:"8px 12px", background:"#f8fafc", borderRadius:6, border:"1px solid #e2e8f0", display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))", gap:8, fontSize:11}}>
                    <div><span style={{color:"#64748b",fontWeight:600}}>Listing:</span> <span style={{color:dl.urgency==="high"?"#dc2626":"#1e293b",fontWeight:700}}>{dl.label}</span></div>
                    <div><span style={{color:"#64748b",fontWeight:600}}>Check-in:</span> <span style={{color:"#1e293b"}}>{rules.checkIn||"Standard"}</span></div>
                    <div><span style={{color:"#64748b",fontWeight:600}}>Dress:</span> <span style={{color:"#1e293b"}}>{rules.dress||"Smart casual"}</span></div>
                  </div>

                  {r.note&&<div style={{fontSize:11,color:"#475569",marginTop:6,fontStyle:"italic"}}>{r.note}</div>}
                  {seatTs&&<div style={{fontSize:9,color:"#64748b",marginTop:4}}>Seats updated {new Date(seatTs).toLocaleString()}</div>}
                </div>
              </div>
              <div style={{background:"#fff",border:`1.5px solid ${tm.border}`,borderRadius:10,padding:"12px 14px",width:"100%",boxSizing:"border-box"}}>
                ...
                {!cm&&<div style={{fontSize:10,color:"#64748b",textAlign:"center",fontStyle:"italic",paddingTop:6,borderTop:"1px solid #e5e7eb"}}>{r.open===null?" Enter open seats":" Enter listed count"}</div>}
              </div>
            </div>
            {r.isDirect?(
              ...
              <div style={{background:"#fff",overflowX:"auto"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                  <thead><tr style={{borderBottom:"1px solid #e5e7eb"}}>
                    {["Connection","Flt","Dep","Aircraft","Arrives","Apt","Cabin","Eligible","Rules","Ticket #","Notes"].map(c=>(<th key={c} style={{padding:"7px 10px",textAlign:"left",fontSize:9,fontWeight:700,letterSpacing:".06em",textTransform:"uppercase",color:"#64748b",whiteSpace:"nowrap"}}>{c}</th>))}
                  </tr></thead>
                  <tbody>
...
function Rules(){
  ...
  return(<div>
    ...
    <p style={{fontSize:12,color:"#64748b",marginBottom:16}}>{J_PARTNERS.length} business class · {Y_PARTNERS.length-J_PARTNERS.length} economy-only · {Y_PARTNERS.length} total</p>
        ...
        return(<div key={code} style={{background:"#ffffff",border:"1px solid #e5e7eb",borderRadius:10,overflow:"hidden"}}>
          <div onClick={()=>setExp(p=>({...p,[code]:!p[code]}))} style={{padding:"12px 16px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10}}>
            <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              ...
              <span style={{fontSize:9,padding:"2px 8px",borderRadius:99,background:ar.j?"#fef3c7":"#f3f4f6",color:ar.j?"#92400e":"#64748b",fontWeight:700}}>{ar.j?"J + Y":"Y only"}</span>
              {ar.alliance&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:99,background:"#f3f4f6",color:"#64748b"}}>{ar.alliance}</span>}
            </div>
          </div>
...
function AirlinePanel({code}){
  ...
  if(!ar)return <div style={{fontSize:11,color:"#64748b"}}>No data for "{code}".</div>;
  ...
  return(
    <div style={{background:"#f5f4f0",border:"1px solid #e5e7eb",borderRadius:9,padding:"12px 14px",fontSize:11}}>
      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,flexWrap:"wrap"}}>
        ...
        <span style={{fontSize:9,padding:"2px 8px",borderRadius:99,background:ar.j?"#fef3c7":"#f3f4f6",color:ar.j?"#92400e":"#64748b",fontWeight:700}}>{ar.j?"Business + Economy":"Economy only"}</span>
        {ar.alliance&&<span style={{fontSize:9,padding:"2px 7px",borderRadius:99,background:"#f3f4f6",color:"#64748b"}}>{ar.alliance}</span>}
      </div>
      {full?(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
          <RI label="How to List" text={full.listingLabel} alert={full.listingAlert}/>
          <RI label="Check-In" text={full.checkIn}/>
...
function RI({label,text,extra,warn,footer,alert}){
  ...
  return(<div>
    <div style={{fontSize:9,fontWeight:700,color:"#64748b",textTransform:"uppercase",letterSpacing:".06em",marginBottom:4,fontFamily:"monospace"}}>{label}</div>
    {als?<div style={{fontSize:11,color:als.color,lineHeight:1.55,padding:"4px 8px",borderRadius:6,background:als.bg,border:`1px solid ${als.border}`}}>{text}</div>
      :<div style={{fontSize:11,color:warn?"#f87171":"#1e293b",lineHeight:1.55}}>{text}</div>}
